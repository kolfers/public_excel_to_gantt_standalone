...
- **Interact Directly:** Click any task bar to open the Edit Task modal. Hovering a bar reveals ✎ Edit and 💬 Comments buttons. The same shortcuts work by hovering task rows in the left panel.
- **Add Native Tasks:** Use the global `Add +` button, hover a Phase/Category row to reveal an inline `+` button, or press `n` while hovering a Phase or Category row.
- **Chaining & Duplication:** In the Edit Task modal, use `"Dependent Task"` to create a pre-linked follow-on (no comments copied), or `"Duplicate Task"` to copy the task with all its comments. The view scrolls to the newly created task automatically.
- **Comments:** Each task supports rich comments with author names, timestamps, and inline edit/delete. The `💬` button on bar hover (or keyboard `c`) opens the Comments panel directly.
- **Keyboard Shortcuts:** `e` Edit · `c` Comments · `d` Dependent · `n` Duplicate (hovering a bar or left-panel row). On Phase/Category hover: `n` adds a task, `e` or `r` renames. `s` saves the Excel from anywhere.
- **Data Filtering:** Click "Active" (excludes On Hold tasks) or "Overdue" to cut through the noise, hiding finished projects and automatically collapsing empty categories. Use the **Filter** button to open a chip-based filter panel for fine-grained filtering by Status, Priority, Comments, Dependency, and Overdue state. Toggle "Compact" mode to compress the left menu and use space efficiently.

### History, Feedback & Archiving
- **Comparison Engine:** Once snapshots are tracked in the Log sheet, use the **Compare** dropdown. The tool overlays older snapshots as ghost tracks (light grey items), and the timeline header glows red. Changed items are visually demarcated.
- **Stats Dashboard:** A slide-out panel automatically tallies milestones achieved, items finished, items put on hold, and overruns that occurred since your chosen baseline snapshot.
